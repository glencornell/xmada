...
package body Model.Widget_Instances_Ordering is
   ...
   Loop_In_Graph_Table_Initial    : constant := 200;
   Loop_In_Graph_Table_Increment  : constant := 200;

   package Loop_In_Graph is new GNAT.Table (
     Table_Component_Type => Node_Is_Contained_In_Loop,
     Table_Index_Type     => Natural,
     Table_Low_Bound      => Natural'First + 1,
     Table_Initial        => Loop_In_Graph_Table_Initial,
     Table_Increment      => Loop_In_Graph_Table_Increment);

   Widget_Instances_Table_Initial    : constant := 200;
   Widget_Instances_Table_Increment  : constant := 200;

   package Widget_Instances is new GNAT.Table (
     Table_Component_Type => Widget_Instance,
     Table_Index_Type     => Widget_Id,
     Table_Low_Bound      => Widget_Id'First + 1,
     Table_Initial        => Widget_Instances_Table_Initial,
     Table_Increment      => Widget_Instances_Table_Increment;
   ...
   Links_Table_Initial    : constant := 200;
   Links_Table_Increment  : constant := 200;

   package Links is new GNAT.Table (
     Table_Component_Type => Successor_Link,
     Table_Index_Type     => Successor_Id,
     Table_Low_Bound      => Successor_Id'First + 1,
     Table_Initial        => Links_Table_Initial,
     Table_Increment      => Links_Table_Increment);
   ...
   Widget_Node_Table_Initial    : constant := 200;
   Widget_Node_Table_Increment  : constant := 200;

   package Widget_Node is new GNAT.Table (
     Table_Component_Type => Widget_Node_Record,
     Table_Index_Type     => Widget_Id,
     Table_Low_Bound      => Widget_Id'First + 1,
     Table_Initial        => Widget_Node_Table_Initial,
     Table_Increment      => Widget_Node_Table_Increment);
   ...
   --!   <Exceptions>
   ---------------------------------------------------------------------------
   procedure Build_Link (Before : Widget_Id; After  : Widget_Id);
   ...
   --!   <Exceptions>
   ---------------------------------------------------------------------------
   procedure Choose (Chosen : Widget_Id);
   ...
   --!   <Exceptions>
   ---------------------------------------------------------------------------
   procedure Gather_Dependencies (Root_Widget : Widget_Id);

   ---------------------------------------------------------------------------
   --! <Subprogram>
   --!   <Unit> ReIndexation
   --!   <Purpose>
   --!       Store the value identifier of Widget
   --!   <Exceptions>
   ---------------------------------------------------------------------------
   procedure ReIndexation (Root_Node : Node_Id);

   ---------------------------------------------------------------------------
   --! <Subprogram>
   --!   <Unit> Initialization
   --!   <Purpose>
   --!      Building the Links and Widget_Node tables
   --!   <Exceptions>
   ---------------------------------------------------------------------------
   ...
   --!    <ImplementationNotes>
   ---------------------------------------------------------------------------
   procedure Choose (Chosen : Widget_Id) is
      Successor : Successor_Id;
      Widget : Widget_Id;

   begin
      Widget_Instances_Order.Increment_Last;
      Widget_Instances_Order.Table (Widget_Instances_Order.Last) :=
                                       Widget_Instances.Table (Chosen).Widget;
      if No_Predecessors = Chosen then
         No_Predecessors :=
                              Widget_Node.Table (Chosen).Next_No_Predecessors;
      end if;
   ...
   procedure Diagnose_Problem is
      ...
      --!   <Exceptions>
      ------------------------------------------------------------------------
      function Find_Path (WidgetFrom, WidgetTo : Widget_Id;
                          MultiPathLength : Natural)
        return Boolean;
      ...
      is
         ...
         --!   <Exceptions>
         ---------------------------------------------------------------------
         function Find_Link (Widget : Widget_Id; PathLenght : Natural)
           return Boolean;
         ...
         --!    <ImplementationNotes>
         ---------------------------------------------------------------------
         function Find_Link (Widget : Widget_Id; PathLength : Natural) 
           return Boolean
         is
            Successor : Successor_Id;
         ...
         begin
            ...
            if Widget = WidgetTo and then PathLength >= MultiPathLength then
               return True;
            ...
            --  Otherwise mark as visited and look at all successors
               ...
               Loop_In_Graph.Table (Loop_In_Graph.Last).Widget :=
                                       Widget_Instances.Table (Widget).Widget;

               while Successor /= No_Successor loop
                  if Find_Link (Links.Table (Successor).After, PathLength + 1)
                  then
                     return True;
                  end if;
                  Successor := Links.Table (Successor).Next;
      ...
      begin
         ...
         return Find_Link (WidgetFrom, 0);
      end Find_Path;
   --  Start of processing for Diagnose_Error
   begin
      for J in Widget_Instances.First .. Widget_Instances.Last loop
         if Widget_Node.Table (J).Widget_Position = 0 then
            if Find_Path (J, J, 1) then
               Put_Line (" The loop is generated by nodes ");
               for K in Loop_In_Graph.First .. Loop_In_Graph.Last loop
                  Put (Integer(Loop_In_Graph.Table (K).Widget));
               end loop;
               Loop_In_Graph.Free;
               Widget_Instances.Free;
               Widget_Instances_Order.Free;
               raise Program_Error;
            end if;
         end if;
      end loop;
   ...
   --!    <ImplementationNotes>
   ---------------------------------------------------------------------------
   procedure ReIndexation (Root_Node : Node_Id) is
      Widget : Node_Id;
   begin
      Widget_Instances.Increment_Last;
      Widget_Instances.Table (Widget_Instances.Last).Widget := Root_Node;

      if Node_Table.Table (Root_Node).Children /= Null_List then
         Widget := First(Node_Table.Table (Root_Node).Children);

         while Widget /= Null_Node loop
            Gather_Dependencies (Widget);
            Widget := Next (Widget);
         end loop;
      end if;
   end ReIndexation;
   ...
   --!    <ImplementationNotes>
   ---------------------------------------------------------------------------
   procedure Find_Widget_Instances_Order (Root_Widget : Node_Id) is
      Widget : Widget_Id;

   begin
      --  Initialize all structures
       ReIndexation (Root_Widget);
       Initialization;
      ...
      Outer : loop
         ...
         --  removing it from the graph and continue
          ...
          exit No_Predecessors_Search when Widget = Null_Widget or else
                           Widget_Node.Table (Widget).Number_Predecessors = 0;
   ...
   procedure Gather_Dependencies is
      Widget              : Node_Id;
      Resource            : Node_Id;
      Constraint_Resource : Node_Id;
      Type_Resource       : Node_Id;

   begin
      --  Loop through all widgets
      for J in Widget_Instances.First .. Widget_Instances.Last loop
         if Node_Table.Table (J).Resources /= Null_List then
            Resource := First(Node_Table.Table (J).Resources);

            while Resource /= Null_Node loop
               Type_Resource := Resource_Type (Resource);
               if Type_Resource = Xt_Motif_Widget_Reference_Resource_Type then
                  Build_Link (Resource_Value(Resource), J);
               end if;
               Resource := Next (Resource);
            end loop;
         end if;

         if Node_Table.Table (J).Constraint_Resources /= Null_List then
            Constraint_Resource
                     := First(Node_Table.Table (J).Constraint_Resources);

            while Constraint_Resource /= Null_Node loop
               Type_Resource := Resource_Type (Constraint_Resource);
               if Type_Resource = Xt_Motif_Widget_Reference_Resource_Type then
                  Build_Link (Resource_Value(Constraint_Resource), J);
               end if;
               Constraint_Resource := Next (Constraint_Resource);
            end loop;
         end if;

         if Node_Table.Table (W).Children /= Null_List then
            Widget := First(Node_Table.Table (J).Children);
